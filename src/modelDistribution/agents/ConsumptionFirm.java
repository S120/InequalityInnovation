...
//TODO check whether targetStock and its getters/setters are really used.
	...
	protected int getRequiredWorkers() {
		...
		return (int) Math.ceil(requiredWorkers);
	}
...
//			TreeMap<Double,ArrayList<CapitalGood>> orderedCapital = new TreeMap<Double,ArrayList<CapitalGood>>();
	...
	public void payTaxes(Item account) {
		...
		double taxes=0;
		if (!this.defaulted){
		taxes=strategy.computeTaxes(government);
		}
		else{
			taxes=0;
		}
		if (Double.isNaN(taxes)){
		...
		if(taxes>liquidity){
			//System.out.println("Default "+ this.getAgentId()+ " con firm due to taxes");
			bankruptcy();
			...
			if(depValue<taxes){
				...
				if (Double.isNaN(cash.getValue()-cashTr)){
					System.out.println("Blah");
				}
	...
	protected void payInterests() {
		...
		//Else, the firm defaults
			...
			System.out.println("Deafault " + this.getAgentId() +" cons firm due to debt service");
			this.bankruptcy();
		}
	}
