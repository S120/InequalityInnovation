...
import modelDistribution.strategies.TaxPayerStrategyWithDeficitManagement;
...
public class HouseholdsDifferentiated extends AbstractHousehold implements GoodDemander, LaborSupplier,
	...
	private void computeWage() {
		...
		if(this.employer==null && this.populationId==StaticValues.WORKERS_ID){
			this.setActive(true, StaticValues.MKT_LABOR);
		}
		else if(this.employer==null && this.populationId==StaticValues.MANAGERS_ID){
			this.setActive(true, StaticValues.MKT_LABORMANAGERS);
		}
		else if(this.employer==null && this.populationId==StaticValues.TOPMANAGERS_ID){
			this.setActive(true, StaticValues.MKT_LABORTOPMANAGERS);
		}
		else if(this.employer==null && this.populationId==StaticValues.RESEARCHERS_ID){
	...
	public void payTaxes(Item account) {
		Government government= (Government) account.getAssetHolder();
		TaxPayerStrategyWithDeficitManagement strategy = (TaxPayerStrategyWithDeficitManagement)this.getStrategy(StaticValues.STRATEGY_TAXES);
		double taxes=strategy.computeTaxes(government);
	...
	public double getNetIncome() {
			...
			TaxPayerStrategyWithDeficitManagement strategy = (TaxPayerStrategyWithDeficitManagement)this.getStrategy(StaticValues.STRATEGY_TAXES);
			double taxes=strategy.computeTaxes(gov);
			...
			for(Agent agent:households.getAgents()){
				...
				averageWage=gov.getAggregateValue(StaticValues.LAG_AVERAGEWORKERSWAGE, 1);
	...
	public double getMicroReferenceVariableForWage() {
		//old code:
		/*if(this.employer==null){
			double averageEmployment = 0;
			for(int i=1; i<=this.employmentWageLag;i++){
				averageEmployment+=this.getPassedValue(StaticValues.LAG_EMPLOYED, i);
			}
			return (1-averageEmployment/employmentWageLag);
		}
		else
			return 0;*/
		double averageEmployment = 0;
	...
	public double getGrossIncome(){
		return this.getWagesPaid()+this.interestsReceived+this.dividendsReceived;
	}
	...
	public void setLaborActive(boolean active) {
		if (this.getPopulationId()==StaticValues.WORKERS_ID){
			this.setActive(active,StaticValues.MKT_LABOR);
		}
		else if (this.getPopulationId()==StaticValues.MANAGERS_ID){
			this.setActive(active,StaticValues.MKT_LABORMANAGERS);
		}
		else if (this.getPopulationId()==StaticValues.TOPMANAGERS_ID){
			this.setActive(active,StaticValues.MKT_LABORTOPMANAGERS);
		}
		else if (this.getPopulationId()==StaticValues.RESEARCHERS_ID){
			this.setActive(active,StaticValues.MKT_LABORRESEARCHERS);
		}
	}
