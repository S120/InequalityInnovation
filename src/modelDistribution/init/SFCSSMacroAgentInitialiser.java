...
public class SFCSSMacroAgentInitialiser extends AbstractMacroAgentInitialiser implements MacroAgentInitialiser{
	...
	public void initialise(MacroPopulation population, MacroSimulation sim) {
		...
		//This allows to control the seed so that the i-th MC repetition of an experiment
		//always starts with the same seed and every MC repetition have a different seed.
		...
		double dividendsPerTopManager=this.dividendsReceived/topManagerSize;
		double workersWage=workersShareIncome*(averageWage/shareWorkers+dividendsPerTopManager*shareTopManagers/shareWorkers);
		double managerWage=managerShareIncome*(averageWage/shareManagers+dividendsPerTopManager*shareTopManagers/shareManagers);
		double topManagerWage=topManagerShareIncome*averageWage/shareTopManagers+dividendsPerTopManager*(topManagerShareIncome-1);
        ...
        govt.setAggregateValue(StaticValues.LAG_WORKERSHAREINCOME, workersShareIncome);
        govt.setAggregateValue(StaticValues.LAG_WORKERSHAREWEALTH, workersShareWealth);
        govt.setAggregateValue(StaticValues.LAG_RESEARCHERSMANAGERSHAREINCOME, managerShareIncome);
        govt.setAggregateValue(StaticValues.LAG_RESEARCHERSMANAGERSHAREWEALTH, managerResearchersShareWealth);
        govt.setAggregateValue(StaticValues.LAG_TOPMANAGERSHAREINCOME, topManagerShareIncome);
        govt.setAggregateValue(StaticValues.LAG_TOPMANAGERSHAREWEALTH, topManagerShareWealth);
		//Households
		
		double workerCons = this.csSales/(workerSize*cPrice)*shareWorkers;
		double managerCons = this.csSales/(managerSize*cPrice)*(shareManagers-shareResearchers);
		double topManagerCons = this.csSales/(topManagerSize*cPrice)*(shareTopManagers);
		double researcherCons=this.csSales/(researcherSize*cPrice)*shareResearchers;
		...
		int kWorkers=(int) Math.round(kEmpl*shareWorkers);
		int kManagers=(int) Math.round(kEmpl*(this.shareManagers-this.shareResearchers));
		int kTopManagers=(int) Math.round(kEmpl*this.shareTopManagers);
		int kResearchers=kEmpl-kManagers-kTopManagers-kWorkers;
		double laborProductivity= kOutput/kWorkers;
		/*int kManagers=(int) Math.round(kEmpl*(this.shareManagers-this.shareResearchers));
		int kTopManagers=(int) Math.round(kEmpl*this.shareTopManagers);
		int kResearchers=(int) Math.round(kEmpl*this.shareResearchers);
		...
		govt.setAggregateValue(StaticValues.LAG_AVERAGEWORKERSWAGE,workersWage);
	    ...
	    ProgressiveTaxRatesComputerStrategyWithDeficitManagement taxRateComputerStrategy= (ProgressiveTaxRatesComputerStrategyWithDeficitManagement) govt.getStrategy(StaticValues.STRATEGY_TAXRATECOMPUTER);
