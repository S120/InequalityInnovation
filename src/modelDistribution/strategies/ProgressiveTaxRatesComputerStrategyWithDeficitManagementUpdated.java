...
public class ProgressiveTaxRatesComputerStrategyWithDeficitManagementUpdated extends ProgressiveTaxRatesComputerStrategyWithDeficitManagement {
	...
	private int lag;
	private double oldTaxRateMultiplyingFactor;
	/* (non-Javadoc)
	...
	public void computeTaxRates() {
		...
		if (strategy==1){
			...
			double incomeTaxRateWorkers=incomeTaxRate*Math.pow(workerShareIncome/workersSharePopulation,progressivenessExponent);
	        gov.setWorkersIncomeTaxRate(incomeTaxRateWorkers*taxRatesMultiplyingFactor);
			double incomeTaxRateManagers=incomeTaxRate*Math.pow(managerResearcherShareIncome/managersResearchersSharePopulation,progressivenessExponent);
	        gov.setManagersIncomeTaxRate(incomeTaxRateManagers*taxRatesMultiplyingFactor);
			double incomeTaxRateTopManagers=incomeTaxRate*Math.pow(topManagerShareIncome/topManagersSharePopulation, progressivenessExponent);
	        gov.setTopManagersIncomeTaxRate(incomeTaxRateTopManagers*taxRatesMultiplyingFactor);
			double wealthTaxRateWorkers=wealthTaxRate*Math.pow(workerShareWealth/workersSharePopulation,progressivenessExponent);
	        gov.setWorkersWealthTaxRate(wealthTaxRateWorkers*taxRatesMultiplyingFactor);
			double wealthTaxRateManagers=wealthTaxRate*Math.pow(managerResearcherShareWealth/managersResearchersSharePopulation,progressivenessExponent);
	        gov.setManagersWealthTaxRate(wealthTaxRateManagers*taxRatesMultiplyingFactor);
			double wealthTaxRateTopManagers=wealthTaxRate*Math.pow(topManagerShareWealth/topManagersSharePopulation, progressivenessExponent);
		    gov.setTopManagersWealthTaxRate(wealthTaxRateTopManagers*taxRatesMultiplyingFactor);
		}
	    }
	    if (sim.getRound()>0 && fixed==true){
	    	gov.setWorkersIncomeTaxRate(gov.getWorkersIncomeTaxRate()/oldTaxRateMultiplyingFactor*taxRatesMultiplyingFactor);
	    	gov.setManagersIncomeTaxRate(gov.getManagersIncomeTaxRate()/oldTaxRateMultiplyingFactor*taxRatesMultiplyingFactor);
	    	gov.setTopManagersIncomeTaxRate(gov.getTopManagersIncomeTaxRate()/oldTaxRateMultiplyingFactor*taxRatesMultiplyingFactor);
	    	gov.setWorkersWealthTaxRate(gov.getWorkersWealthTaxRate()/oldTaxRateMultiplyingFactor*taxRatesMultiplyingFactor);
	    	gov.setManagersWealthTaxRate(gov.getManagersWealthTaxRate()/oldTaxRateMultiplyingFactor*taxRatesMultiplyingFactor);
	    	gov.setTopManagersWealthTaxRate(gov.getTopManagersWealthTaxRate()/oldTaxRateMultiplyingFactor*taxRatesMultiplyingFactor);
	    }
	    oldTaxRateMultiplyingFactor=taxRatesMultiplyingFactor;
	}
