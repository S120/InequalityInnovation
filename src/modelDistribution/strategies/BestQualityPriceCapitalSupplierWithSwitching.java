...
import jmab.strategies.SwitchingStrategy;
...
public class BestQualityPriceCapitalSupplierWithSwitching extends AbstractStrategy implements
	...
	public MacroAgent selectGoodSupplier(ArrayList<Agent> sellers,
		...
		ConsumptionFirmWithHierarchy firm= (ConsumptionFirmWithHierarchy) this.getAgent();
		...
		for (Agent agent:sellers){
			...
			double laborProductivity=capitalOffered.getProductivity()*capitalOffered.getCapitalLaborRatio()/((firm.getShareWorkers()+firm.getShareManagers()+firm.getShareTopManagers())/firm.getShareWorkers());
					//laborProductivity=capitalOffered.getProductivity()*capitalOffered.getCapitalLaborRatio();
			double cost=((expectedWages/laborProductivity)*capitalOffered.getCapitalDuration()+capitalOffered.getPrice());
			if (cost<minCost){
				bestSupplier=seller;
				minCost=cost;
			}
		...
		if (!previousSupplier.isDead()&&previousSupplier.isActive(((MacroSimulation)((SimulationController)this.scheduler).getSimulation()).getActiveMarketId())){
			...
			double laborProductivity=capitalOffered.getProductivity()*capitalOffered.getCapitalLaborRatio()/((firm.getShareWorkers()+firm.getShareManagers()+firm.getShareTopManagers())/firm.getShareWorkers());
			previousCost=((expectedWages/laborProductivity)*capitalOffered.getCapitalDuration()+capitalOffered.getPrice());
		}
		else{
			previousCost=Double.POSITIVE_INFINITY;
		}
		if(previousCost>minCost){
			if(previousCost==Double.POSITIVE_INFINITY||strategy.switches(previousCost,minCost)){
