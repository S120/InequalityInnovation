...
public class AdaptiveMarkUpOnACUpdated extends AbstractStrategy implements
		PricingStrategy {
	...
	public double computePrice() {
		...
		if (seller instanceof ConsumptionFirmWithHierarchy){
			ConsumptionFirmWithHierarchy firm= (ConsumptionFirmWithHierarchy) seller;
			...
			if (macroSim.getRound()>1){
				double capacity = firm.getPassedValue(StaticValues.LAG_CAPACITY, 1);
				double production = firm.getPassedValue(StaticValues.LAG_PRODUCTION, 1);
			...
			CapitalFirmWithHierarchy firm= (CapitalFirmWithHierarchy) seller;
			double pastSales=firm.getPassedValue(StaticValues.LAG_REALSALES, 1);
			double pastpastSales=firm.getPassedValue(StaticValues.LAG_REALSALES, 2);
			double realSalesChange= (double)(firm.getPassedValue(StaticValues.LAG_REALSALES, 1)-firm.getPassedValue(StaticValues.LAG_REALSALES, 2))/ (double) firm.getPassedValue(StaticValues.LAG_REALSALES, 2);
