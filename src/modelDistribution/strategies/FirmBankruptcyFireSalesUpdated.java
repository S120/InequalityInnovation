...
public class FirmBankruptcyFireSalesUpdated extends AbstractStrategy implements
		BankruptcyStrategy {
		...
		if (totalDebt!=0){
			...
			if (firm instanceof ConsumptionFirm){
                ...
                if (activeFirms>0 && !Double.isNaN(sectorNW/activeFirms*scalingFactor) && !Double.isInfinite(sectorNW/activeFirms*scalingFactor)){
            		newDeposits=Math.max(0, (sectorNW/activeFirms*scalingFactor)-firm.getNetWealth());
                }
				...
				for (Agent h:owners.getAgents()){
					...
					for(int i=0;i<loans.size();i++){
						...
						double amountToPay=ownersDisbursment*owner.getItemStockMatrix(true, StaticValues.SM_DEP).getValue()/totalOwnersWealth*(banksLosses[i])/totalBanksLoss;
						 if(Double.isNaN(amountToPay)||Double.isInfinite(amountToPay)){
			    				System.out.println("Blah");
			    			}
					...
					if (newDeposits>0){
						...
						double ownerTransfer=newDeposits*owner.getItemStockMatrix(true, StaticValues.SM_DEP).getValue()/totalOwnersWealth;
